...
export default {
    ...
    methods:{
        getList(start){
            ...
            this.$http('alcoholTypeService','findDatas',data)
            .then(res=>{
                console.log(res)
                if(res.rows){
                    var arr = res.rows
        ...
        remove (id) {
            this.$Modal.confirm({
                ...
                onOk: () => {
                     var data = {id:id};
                    this.$http('alcoholTypeService','deleteData',data)
                    .then(res=>{
        ...
        modify(row){//修改
            this.inputValue.product_type=row.product_type
        ...
        newAdd(){
            ...
            var data = {
                product_type:this.inputValue.product_type,
                url:this.pics[0]
            }
            if(this.id){
                data.id = this.id
            }
            this.$http('alcoholTypeService','addOrUpdate',data)
            .then(res=>{
